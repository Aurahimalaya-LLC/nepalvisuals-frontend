...
const UpcomingDepartures = () => {
  ...
  return (
    <section className="mb-10 md:mb-24 px-4 md:px-8 lg:px-16 container mx-auto max-w-7xl">
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-4">
        <div>
          ...
          <p className="text-text-secondary max-w-xl">
            Secure your spot on our most popular expeditions for the upcoming
            season.
          </p>
        </div>
      ...
      <div className="flex flex-col gap-4">
        {departures.map((departure, index) => (
          <div
            key={index}
            className="group bg-surface-dark p-4 md:p-6 rounded-2xl border border-white/5 hover:border-primary/30 transition-all hover:shadow-lg hover:shadow-primary/5 flex flex-col md:flex-row gap-6 items-start md:items-center"
          >
            ...
            <div className="flex flex-col md:flex-row gap-6 md:items-center flex-grow justify-between w-full md:w-auto">
              <div className="flex items-center gap-3">
                <div className="bg-surface-darker px-3 py-2 rounded-xl text-center min-w-[85px] border border-white/10 shadow-sm group-hover:border-primary/20 transition-colors">
                  <span className="block text-[10px] font-bold text-primary uppercase tracking-widest mb-0.5">
                    {departure.startDate.month}
                  </span>
                  <span className="block text-2xl font-black text-white">
                    {departure.startDate.day}
                  </span>
                </div>
                ...
                <div className="bg-surface-darker px-3 py-2 rounded-xl text-center min-w-[85px] border border-white/10 shadow-sm group-hover:border-primary/20 transition-colors">
                  <span className="block text-[10px] font-bold text-primary uppercase tracking-widest mb-0.5">
                    {departure.endDate.month}
                  </span>
                  <span className="block text-2xl font-black text-white">
                    {departure.endDate.day}
                  </span>
                </div>
              </div>
                ...
                <button
                  className={`px-5 py-2.5 rounded-full text-sm font-bold transition-all whitespace-nowrap ${
                    departure.status === "orange"
                      ...
                      : "bg-white/10 hover:bg-primary hover:text-white text-white border border-white/10 hover:border-primary/50"
                  }`}
                >
